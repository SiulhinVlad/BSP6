...
OUTPUT_ROOT  = Path("./output_multimodal_fed_bart")     
...
tokenizer         = BartTokenizerFast.from_pretrained(BART_NAME)

vision_encoder = ViTModel.from_pretrained(VISION_NAME)
bart_decoder   = BartForConditionalGeneration.from_pretrained(BART_NAME)
...
class VisionBartModel(torch.nn.Module):
    ...
    def forward(self, pixel_values=None, input_ids=None,
        ...
        if input_ids is not None:
            input_embeds = self.bart_model.get_encoder().embed_tokens(input_ids)
...
global_model = VisionBartModel(vision_encoder, bart_decoder).to(device)

global_model.bart_model.config.decoder_start_token_id = \
    global_model.bart_model.config.bos_token_id
global_model.bart_model.config.eos_token_id           = \
    global_model.bart_model.config.eos_token_id
global_model.bart_model.config.pad_token_id           = \
    global_model.bart_model.config.pad_token_id
global_model.bart_model.config.vocab_size             = \
    global_model.bart_model.config.vocab_size
global_model.bart_model.config.max_length             = 128
global_model.bart_model.config.no_repeat_ngram_size   = 3
global_model.bart_model.config.early_stopping         = True
global_model.bart_model.config.num_beams              = 4
global_model.bart_model.config.temperature            = 1.0
...
for rnd in range(FED_ROUNDS):
    ...
    for c in range(NUM_CLIENTS):
        ...
        client_output_dir = OUTPUT_ROOT / f"round_{rnd}" / f"client{c}"
    ...
    bart_decoder_dir = round_dir / "bart_decoder"
    bart_decoder_dir.mkdir(exist_ok=True)
    global_model.bart_model.save_pretrained(bart_decoder_dir)

    torch.save(
        {
            "vision_bart_state_dict": global_model.state_dict(),
            "proj_weight":            global_model.proj.weight.detach().cpu().numpy().tolist(),
            "proj_bias":              global_model.proj.bias.detach().cpu().numpy().tolist(),
        },
        round_dir / "vision_bart_full_state.pt",
    )
...
(final_dir / "bart_decoder").mkdir(exist_ok=True)
global_model.bart_model.save_pretrained(final_dir / "bart_decoder")
...
torch.save(
    ...
    final_dir / "vision_bart_full_state.pt",
)
